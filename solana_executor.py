...
class SolanaExecutor:
    ...
    def _sell_jupiter(self, token_address: str, token_amount_pct: float,
                      symbol: str) -> dict:
        ...
        try:
            ...
            if tx_hash and tx_hash != "MIGRATED_TO_RAYDIUM":
                confirmed = self._confirm_transaction(tx_hash)
                if confirmed:
                    log.info(f"✅ VENTE JUPITER — {symbol} | {tx_hash[:20]}...")
                    return {"success": True, "tx_hash": tx_hash,
                            "source": "jupiter"}

        except Exception as e:
            log.error(f"❌ Jupiter sell: {e}")

        return {"success": False, "reason": "jupiter_sell_failed"}
    ...
    def sell_token(self, token_address: str, sell_pct: float = 100,
        ...
        # Fallback Jupiter (tokens Raydium ou tout autre)
        result = self._sell_jupiter(token_address, sell_pct, symbol)
        if result["success"]:
            return result

        log.error("❌ Tous les providers de vente ont échoué")
        return {"success": False, "reason": "Tous providers échoués"}
